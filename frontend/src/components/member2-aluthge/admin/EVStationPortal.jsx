...
const EVStationPortal = () => {
    ...
    const fetchStations = async () => {
        setLoading(true);
        try {
            const res = await axios.get('http://localhost:8081/api/ev-stations');
            setStations(res.data);
            // Optional: keep mirror in localStorage for offline/recommendation
    ...
    const persist = (data) => {
        // This is now handled by individual API calls, but we keep the state sync
        setStations(data);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert('Failed to save station to database.');
        }
    };

    const startEdit = (station) => {
        setForm({ ...station });
        setEditingId(station._id || station.id);
        setShowForm(true);
    };

    const confirmDelete = async () => {
        try {
            await axios.delete(`http://localhost:8081/api/ev-stations/${deleteModal.id}`);
            const updated = stations.filter(s => (s._id || s.id) !== deleteModal.id);
            persist(updated);
        ...
        <div className="w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 text-sm text-slate-500 bg-white border border-slate-200 rounded-xl px-4 py-2 shadow-sm hidden sm:flex">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 text-sm text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl px-4 py-2 shadow-sm font-semibold transition-colors"
                    >
                        <Plus size={16} />
                        <span className="hidden sm:inline">Register New Station</span>
                    ...
                    {showForm && (
                        ...
                        >
                            <motion.div
                                ...
                                className="bg-white rounded-3xl shadow-xl w-full max-w-xl p-7 my-8 relative z-[110]"
                            >
                                <div className="flex items-center justify-between mb-6">
                                    <h2 className="text-xl font-bold text-slate-800">
                                        {editingId ? 'Edit Station' : 'Register New EV Station'}
                                    </h2>
                                    <button onClick={() => { setForm(defaultForm); setEditingId(null); setShowForm(false); }}
                                        ...
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    <span className="text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg font-semibold">{station.id || station._id}</span>
                                                    {station.power && (
                                            ...
                                            <div className="flex gap-2 shrink-0">
                                                ...
                                                <button onClick={() => setDeleteModal({ open: true, id: station._id || station.id })}
